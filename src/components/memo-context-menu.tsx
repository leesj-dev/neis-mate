...
export function MemoContextMenu({ memo, onClose }: MemoContextMenuProps) {
  const { addMemo, deleteMemo, user, updateMemo } = useAppStore();
  ...
  const handleNewVersion = () => {
    const newMemo = createNewVersion(memo);
    addMemo(newMemo);
    setShowMenu(false);
    onClose?.();
  };
  ...
  if (showMenu) {
    return (
      <>
        ...
        />
        <div className="absolute right-0 top-0 z-50 bg-white dark:bg-gray-800 border rounded-md shadow-lg py-1 min-w-[150px]">
          <button
        ...
        <Dialog open={showHistoryDialog} onOpenChange={setShowHistoryDialog}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                파일의 버전 히스토리를 확인하고 이전 버전으로 되돌릴 수 있습니다.
              </DialogDescription>
            </DialogHeader>
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {version.modifiedTime.toLocaleString()}
                        </p>
                      </div>
