...
import { getDownloadFileName, htmlToPlainText } from "./memo-utils";
...
export async function downloadAllMemos(memos: Memo[], folders: Folder[], mode: string): Promise<void> {
  const zip = new JSZip();
  ...
  if (mode === "일반") {
    ...
    memos.forEach((memo) => {
      if (memo.mode === "일반") {
        ...
        if (targetFolder) {
          const filename = getDownloadFileName(memo);
          const textContent = htmlToPlainText(memo.content);
    ...
    gradeGroups.forEach((gradeMemos, grade) => {
      ...
      if (gradeFolder) {
        gradeMemos.forEach((memo) => {
          const filename = getDownloadFileName(memo);
          const textContent = htmlToPlainText(memo.content);
          gradeFolder.file(filename, textContent);
        });
      }
    });
  } else if (mode === "교사") {
            ...
            subjectMap.forEach((subjectMemos, subject) => {
              ...
              if (subjectFolder) {
                subjectMemos.forEach((memo) => {
                  const filename = getDownloadFileName(memo);
                  const textContent = htmlToPlainText(memo.content);
                  subjectFolder.file(filename, textContent);
                });
              }
            });
          }
